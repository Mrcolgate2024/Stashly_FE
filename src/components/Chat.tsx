...
export const Chat: React.FC<ChatProps> = ({ scriptLoaded = false }) => {
  ...
  // If we detect issues, we can toggle avatars off and on
  const reinitializeAvatars = () => {
    setAvatarsVisible(false);
    setTimeout(() => setAvatarsVisible(true), 300);
    setAvatarsReady(0);
  };

  useEffect(() => {
    if (scriptLoaded) {
      console.log("Script loaded, avatars can now initialize");
    }
  }, [scriptLoaded]);
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col gap-4 p-4 relative">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <ChatControls
          ...
          onClear={() => {
            clearMessages();
            // Try reinitializing avatars when chat is cleared
            reinitializeAvatars();
          }}
        />
      </div>
