...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  // Format $tashly to make it uppercase
  const formattedContent = content.replace(/\$tashly/gi, '<span class="stashly-brand">$TASHLY</span>');
  
  return (
    <div className="stashly-markdown">
      <style>{`
        .stashly-brand {
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          p: ({ node, children, ...props }) => {
            return <p {...props} dangerouslySetInnerHTML={{ __html: String(children).replace(/<span class="stashly-brand">(.*?)<\/span>/g, '<span class="stashly-brand">$1</span>') }} />;
          }
        }}
      >
        {formattedContent}
